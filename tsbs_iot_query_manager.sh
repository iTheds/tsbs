...
INFLUX_URLS="${INFLUX_URLS:-http://127.0.0.1:8086}"
INFLUX_DB="${INFLUX_DB:-tsbs_iot}"
QUESTDB_URLS="${QUESTDB_URLS:-http://127.0.0.1:9000/}"
...
# ============== 各后端：生成与执行（use-case=iot） ==============

generate_influx_one() {
  local qtype="$1"
  local outfile="$QUERY_DIR/queries_influx_${qtype}.gz"
  local cmd=("$GEN_BIN" --use-case=iot --seed="$SEED" --scale="$SCALE" \
...
generate_tdengine_one() {
  ...
  # 注意：你的 tsbs 分支如果要求 --format=tdengine（小写），请改之
  local cmd=("$GEN_BIN" --use-case=iot --seed="$SEED" --scale="$SCALE" \
    --timestamp-start="$START" --timestamp-end="$END" \
